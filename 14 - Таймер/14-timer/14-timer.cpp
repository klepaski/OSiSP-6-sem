// 14-timer.cpp : Определяет точку входа для приложения.
//
	...
	_In_ int       nCmdShow)
{
	...
	LoadStringW(hInstance, IDC_MY14TIMER, szWindowClass, MAX_LOADSTRING);
	...
	HACCEL hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_MY14TIMER));
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
	...
	wcex.hIcon = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_MY14TIMER));
	wcex.hCursor = LoadCursor(nullptr, IDC_ARROW);
	wcex.hbrBackground = (HBRUSH)(COLOR_WINDOW + 1);
	wcex.lpszMenuName = MAKEINTRESOURCEW(IDC_MY14TIMER);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
		...
		SetTimer(hWnd, IDT_TIMER2, 100, (TIMERPROC)NULL);
		SetTimer(hWnd, IDT_TIMER4, 1000, (TIMERPROC)NULL);
		SetTimer(hWnd, IDT_TIMER4, 1000, (TIMERPROC)NULL);
		break;
	...
	case WM_TIMER:
		...
		{
			if (t == 2)
				KillTimer(hWnd, IDT_TIMER4);			//int n = 10,  t = 9, m = 10,  el = 0; 
			t--;
